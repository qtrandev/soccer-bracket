...
// Classic black-and-white soccer ball centered at (cx, cy) with given radius.
// Internal coordinate space: 200×200 with ball centered at (100,100), r=95.
// Pentagon patch layout: one center pentagon + 5 edge patches (standard Telstar pattern).
...
// Trophy centered at (cx, cy), scaled by scale factor
...
// 1200×1200 square OG image — winner variant (flag + name centered)
function winnerSvg(championName, flagDataUri) {
  ...
  // Ball (left) and flag (right) share the top row, centred as a pair
  const ballCx = 370, ballCy = 275, ballR = 185;
  const flagW = 360, flagH = 240;
  const flagX = 655, flagY = ballCy - flagH / 2; // vertically centred with ball
...
// 1200×1200 square OG image — generic variant (no winner picked)
function genericSvg() {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1200" width="1200" height="1200">
  <rect width="1200" height="1200" fill="#091a0f"/>
...
export const config = { path: '/og-square' };
